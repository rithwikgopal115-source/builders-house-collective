...
interface ChannelProject {
  ...
  is_active: boolean; is_hidden: boolean; gradient_idx: number | null; created_at: string;
}
...
// ── Shared post loader ────────────────────────────────────────
const loadPosts = async (channelId:string, projectId:string|null|'all'):Promise<FeedPost[]> => {
...
// ── General Tile ──────────────────────────────────────────────
const GeneralTile = ({onClick,style}:{onClick:()=>void;style?:React.CSSProperties}) => {
  const [hov,setHov]=useState(false);
  return (
    <button onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      className="relative overflow-hidden text-left active:scale-[0.98]"
      style={{background:'#000',borderRadius:0,minHeight:180,boxShadow:hov?'0 0 40px rgba(232,115,74,0.15)':'none',transition:'box-shadow 0.35s ease',...style}}>
      <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 30% 70%,rgba(232,115,74,0.08) 0%,transparent 70%)',opacity:hov?1:0,transition:'opacity 0.35s ease',pointerEvents:'none'}}/>
      <div className="relative z-10 p-5 h-full flex flex-col justify-end">
        <p className="text-[10px] font-mono uppercase tracking-widest mb-2" style={{color:'rgba(255,255,255,0.35)'}}>all channels</p>
        <div style={{display:'inline-block',border:'1px solid rgba(255,255,255,0.5)',padding:'4px 12px'}}>
          <span className="text-xl font-bold" style={{color:'#F5F0EB',letterSpacing:'-0.02em'}}>General</span>
        </div>
      </div>
    </button>
  );
    ...
    <div className="relative overflow-hidden" style={{borderRadius:0,minHeight:130,...style}}
      ...
      {isAdmin && hov && (
        <div className="absolute top-2 right-2 z-20 flex gap-1">
          <button onClick={e=>{e.stopPropagation();onEdit();}} className="h-7 w-7 flex items-center justify-center rounded" style={{background:'rgba(0,0,0,0.6)',color:'#F5F0EB'}} title="edit">
          ...
          <button onClick={e=>{e.stopPropagation();onDelete();}} className="h-7 w-7 flex items-center justify-center rounded" style={{background:'rgba(0,0,0,0.6)',color:'#ff6b6b'}} title="delete">
            <Trash2 className="h-3.5 w-3.5"/>
          </button>
        </div>
      )}
      {project.is_hidden&&isAdmin&&(
        <div className="absolute top-2 left-2 z-20 text-[10px] font-mono uppercase tracking-wider px-2 py-0.5" style={{background:'rgba(0,0,0,0.65)',color:'#ff6b6b',border:'1px solid rgba(255,107,107,0.4)'}}>hidden</div>
      )}
    </div>
  );
};
...
const PostFeed = ({channelId,projectId}:{channelId:string;projectId:string|null|'all'}) => {
  ...
  const load=useCallback(async()=>{
    setLoading(true);
    setPosts(await loadPosts(channelId,projectId));
    setLoading(false);
  },[channelId,projectId]);

  useEffect(()=>{load();},[load]);
  useEffect(()=>{
    const ch=supabase.channel(`feed:${channelId}:${String(projectId)}`)
  ...
  if(posts.length===0) return <div className="py-16 text-center text-sm font-mono" style={{background:'#161616',border:'1px solid rgba(255,255,255,0.06)',borderRadius:16,color:'#6A6460'}}>nothing here yet — be the first.</div>;
          ...
          <div key={p.id} className="relative group">
            ...
            {(p.user_id===user?.id||isAdmin)&&(
              <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                {p.user_id===user?.id&&(
                  <button onClick={()=>setEditingPost(p)} className="h-7 w-7 flex items-center justify-center rounded-md hover:bg-white/10" style={{color:'#A09890'}} title="edit">
                    <Pencil className="h-3.5 w-3.5"/>
                  </button>
                )}
                <button onClick={()=>deletePost(p.id)} className="h-7 w-7 flex items-center justify-center rounded-md hover:bg-red-500/20" style={{color:'#A09890'}} title="delete">
                  <Trash2 className="h-3.5 w-3.5"/>
                </button>
              </div>
            )}
...
// ── General View ──────────────────────────────────────────────
const GeneralView = ({channel,onBack}:{channel:Channel;onBack:()=>void}) => (
  <div>
    <button onClick={onBack} className="inline-flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider mb-4 hover:text-primary" style={{color:'#A09890'}}>
      <ArrowLeft className="h-3.5 w-3.5"/>{channel.name.toLowerCase()}
    </button>
    <div className="mb-5 p-5" style={{background:'#000',borderRadius:0}}>
      <p className="text-[10px] font-mono uppercase tracking-widest mb-1" style={{color:'rgba(255,255,255,0.3)'}}>{channel.name.toLowerCase()}</p>
      <div style={{display:'inline-block',border:'1px solid rgba(255,255,255,0.5)',padding:'4px 12px'}}>
    ...
    <Tabs defaultValue="all">
      ...
      <TabsContent value="all"><PostFeed channelId={channel.id} projectId="all"/></TabsContent>
      <TabsContent value="general"><PostFeed channelId={channel.id} projectId={null}/></TabsContent>
    </Tabs>
...
const ProjectModal = ({open,onOpenChange,onSaved,existing,nextSlot}:{
  open:boolean;onOpenChange:(o:boolean)=>void;onSaved:()=>void;
  existing?:ChannelProject|null;nextSlot:number;
}) => {
  const {user}=useAuth();
  ...
  useEffect(()=>{
    if(existing){setName(existing.name);setDesc(existing.description??'');setGradIdx(existing.gradient_idx??existing.slot_number-1);}
    else{setName('');setDesc('');setGradIdx(nextSlot>0?nextSlot-1:0);}
  ...
  const save=async()=>{
    ...
    if(isEdit&&existing){
      ...
      ({error}=await supabase.from('channel_projects').insert({slot_number:nextSlot,name:name.trim(),description:desc.trim()||null,gradient_idx:gradIdx,is_active:true,is_hidden:false,created_by:user?.id}));
    }
    setBusy(false);
    if(error){toast.error(error.message);return;}
    toast.success(isEdit?'project updated':`"${name.trim()}" created`);
    onOpenChange(false);onSaved();
  };

  const previewIdx=Math.min(gradIdx,29);
  const g=GRADIENTS[previewIdx]; const tc=getTextColor(previewIdx);
  ...
  return (
    <Dialog open={open} onOpenChange={o=>{if(!o){setName('');setDesc('');}onOpenChange(o);}}>
      <DialogContent className="border-0 max-w-md max-h-[90vh] overflow-y-auto" style={{background:'#161616',border:'1px solid rgba(255,255,255,0.06)',borderRadius:16}}>
        ...
        {/* Preview */}
        <div className="relative overflow-hidden flex items-end p-4 mb-1" style={{background:gradientCss(g.base),borderRadius:8,height:80}}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button onClick={save} disabled={busy} className="px-6">{busy?(isEdit?'saving…':'creating…'):(isEdit?'save changes':'create project')}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
// ── Tile Grid ─────────────────────────────────────────────────
const TileGridView = ({channel,projects,isAdmin,onSelectGeneral,onSelectProject,onAddProject,onRefresh}:{
  channel:Channel;projects:ChannelProject[];isAdmin:boolean;
  onSelectGeneral:()=>void;onSelectProject:(p:ChannelProject)=>void;
  onAddProject:()=>void;onRefresh:()=>void;onEditProject:(p:ChannelProject)=>void;
} & {onEditProject:(p:ChannelProject)=>void}) => {
  const active=projects.filter(p=>p.is_active&&(isAdmin||!p.is_hidden));
  const used=projects.map(p=>p.slot_number);
  const nextSlot=Array.from({length:30},(_,i)=>i+1).find(n=>!used.includes(n))??0;

  const hide=async(p:ChannelProject)=>{
    const {error}=await supabase.from('channel_projects').update({is_hidden:!p.is_hidden}).eq('id',p.id);
    if(error){toast.error(error.message);return;}
    toast.success(p.is_hidden?'project visible':'project hidden');onRefresh();
  };
  const del=async(p:ChannelProject)=>{
    if(!window.confirm(`Delete "${p.name}"?`))return;
    const {error}=await supabase.from('channel_projects').update({is_active:false}).eq('id',p.id);
    if(error){toast.error(error.message);return;}
    toast.success('project removed');onRefresh();
  };

  return (
    <>
      <style>{`.ptg{display:grid;grid-template-columns:repeat(4,1fr);gap:3px}.ptg-gen{grid-column:span 2;grid-row:span 2;min-height:260px}@media(max-width:640px){.ptg{grid-template-columns:repeat(2,1fr)}}`}</style>
      <div className="ptg">
        <GeneralTile onClick={onSelectGeneral} style={{gridColumn:'span 2',gridRow:'span 2',minHeight:260}}/>
        {active.map(p=>(
          <ProjectTile key={p.id} project={p} isAdmin={isAdmin}
            onClick={()=>onSelectProject(p)} onEdit={()=>onEditProject(p)}
            onHide={()=>hide(p)} onDelete={()=>del(p)}/>
        ))}
        {isAdmin&&nextSlot>0&&<AddProjectTile onClick={onAddProject} nextSlot={nextSlot}/>}
      </div>
    </>
  );
};
...
export const ProjectChannelPage = ({channel}:{channel:Channel}) => {
  ...
  const [editingProject,setEditingProject]=useState<ChannelProject|null>(null);
  ...
  useEffect(()=>{load();},[load]);
  useEffect(()=>{
    const ch=supabase.channel('cp_rt').on('postgres_changes',{event:'*',schema:'public',table:'channel_projects'},load).subscribe();
    return ()=>{supabase.removeChannel(ch);};
  },[load]);

  if(view==='general') return <GeneralView channel={channel} onBack={()=>setView('grid')}/>;
  if(view==='project'&&selected) return <ProjectView channel={channel} project={selected} onBack={()=>{setView('grid');setSelected(null);}}/>;

  return (
    <>
      <TileGridView
        channel={channel} projects={projects} isAdmin={isAdmin}
        onSelectGeneral={()=>setView('general')}
        onSelectProject={p=>{setSelected(p);setView('project');}}
        onAddProject={()=>setShowAdd(true)}
        onEditProject={p=>setEditingProject(p)}
        onRefresh={load}
      />
      {/* Add modal */}
      <ProjectModal open={showAdd} onOpenChange={setShowAdd} onSaved={load} nextSlot={nextSlot}/>
      {/* Edit modal */}
      <ProjectModal open={!!editingProject} onOpenChange={o=>{if(!o)setEditingProject(null);}} onSaved={()=>{setEditingProject(null);load();}} existing={editingProject} nextSlot={nextSlot}/>
    </>
  );
};
